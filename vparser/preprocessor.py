...
def pp(commandfile: str) -> str:
    if not os.path.exists(commandfile):
        raise FileNotFoundError(f"{commandfile} doesn't exist")

    # os.getenv returns None if the env var is not found
    # assume that iverilog is in $PATH
    iverilog_bin = os.getenv("PYVERILOG_IVERILOG") or "iverilog"
    cmd = [iverilog_bin]
    # Only preprocess
    cmd.append("-E")
    # commandfile
